...
// font with different metrics, causing overflow warnings. This suppresses that
// layout warning without masking real errors.
...
const _uid = 'test-uid-123'; // matches FakeAuthUser.uid
  ...
  group('LoginScreen — successful sign-in routing', () {
    testWidgets('driver role navigates to /dashboard', (tester) async {
      when(mockAuth.signInWithEmailPassword(email: _email, password: _pass))
          .thenAnswer((_) async => const FakeAuthUser());
      when(mockAuth.currentUser).thenReturn(const FakeAuthUser());
